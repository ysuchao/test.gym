...
state_std = np.array([1, 1, 0.2, 0.3])
...
class DQN(torch.nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.state_value_net = torch.nn.Sequential(
            torch.nn.Linear(128, 64),
            torch.nn.ReLU(),
            torch.nn.Linear(64, 1),
        )
        self.action_state_net = torch.nn.Sequential(
            torch.nn.Linear(128, 64),
            torch.nn.ReLU(),
    ...
    def forward(self, x):
        ...
        state_values = self.state_value_net(features)
        action_values = self.action_state_net(features)
        q_values = state_values + (action_values - action_values.mean(dim=1, keepdim=True))
        return q_values
...
def export_onnx(policy_net, filename="cartpole-double-dueling-dqn.onnx"):
    policy_net.eval()
